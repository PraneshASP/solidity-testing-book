...
Boundary testing focuses on verifying behavior at the exact edges of valid and invalid input ranges. Testing exactly one unit above and one unit below a boundary is more intuitive and it gives you high confidence that the check is implemented correctly (using `>` vs `>=`, etc.) with minimal test overhead.
